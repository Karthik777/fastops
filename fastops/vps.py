...
from fastcore.all import L, listify
from fastcloudinit.core import cloud_init_config
from .docker import Cli, Compose
...
def vps_init(hostname, pub_keys, username='deploy', docker=True,
    ...
    rcmds = list(cmds or [])
    if docker: rcmds = [c.format(username=username) for c in _DOCKER_CMDS] + rcmds
    if cf_token:
        rcmds += [
    ...
    return cloud_init_config(hostname=hostname, username=username, pub_keys=pub_keys,
                             packages=['curl'] + listify(packages), cmds=rcmds, **kw)
...
def hcloud_auth(token, name='default'):
    'Write hcloud CLI config with API token'
    cfg = Path.home() / '.config' / 'hcloud' / 'cli.toml'
    cfg.parent.mkdir(parents=True, exist_ok=True)
    cfg.write_text(f'active_context = "{name}"\n\n[[contexts]]\n  name = "{name}"\n  token = "{token}"\n')

def create(name, image='ubuntu-24.04', server_type='cx22', location=None, cloud_init=None, ssh_keys=None):
    'Create a Hetzner server. cloud_init: YAML string or file path. Returns IP.'
    args = ['--name', name, '--image', image, '--type', server_type]
    if location: args += ['--location', location]
    for k in listify(ssh_keys): args += ['--ssh-key', k]
    tmp = None
    if cloud_init:
        if os.path.isfile(str(cloud_init)): args += ['--user-data-from-file', str(cloud_init)]
        else:
...
def server_ip(name) -> str:
    ...
    return json.loads(callhcloud('server', 'describe', name, '-o', 'json'))['public_net']['ipv4']['ip']
...
def sync(src, dst_path, host, user='deploy', key=None, port=22):
    ...
    args = ['rsync', '-az', '--delete'] + (['-e', f'ssh -i {key}'] if key else [])
    subprocess.run(args + [str(src), f'{user}@{host}:{dst_path}'], check=True)

def deploy(compose, host, user='deploy', key=None, path='/srv/app',
           pull=False, packages=None, pip_pkgs=None):
    ...
    with tempfile.NamedTemporaryFile(suffix='.yml', mode='w', delete=False) as f:
        f.write(str(compose) if isinstance(compose, Compose) else compose); tmp = f.name
    try: sync(tmp, f'{path}/docker-compose.yml', host, user=user, key=key)
    finally: Path(tmp).unlink(missing_ok=True)
    cmds = []
    if packages: cmds.append(f'sudo apt-get install -y {" ".join(listify(packages))}')
    if pip_pkgs:  cmds.append(f'pip install {" ".join(listify(pip_pkgs))}')
    if pull:      cmds.append(f'cd {path} && docker compose pull')
    cmds.append(f'cd {path} && docker compose up -d')
    return run_ssh(host, *cmds, user=user, key=key)
