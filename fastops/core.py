...
import re, json, os, yaml, keyring
...
def drun(img:str, detach=False, ports=None, name=None, remove=False, command=None):
	'Run a container, return container ID (detached) or output'
	p_flags = concat(['-p', f'{hp}:{cp.split("/")[0]}'] for cp, hp in (ports or {}).items())
	return Docker()('run', *p_flags, *mk_flags(d=detach, rm=remove, name=name), str(img), *listify(command or []))
